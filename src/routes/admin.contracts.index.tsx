...
import {
  ...
  CheckCircle2, RotateCcw, Ban, LayoutGrid, TableIcon, Download,
} from "lucide-react";
...
type FilterKey = "all" | "15" | "30" | "45" | "65" | "90" | "expired" | "cancelled";
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div></div>
        <a
          href="/templates/employment-services-agreement.pdf"
          download
          className="inline-flex items-center gap-2 rounded-full bg-gradient-brand px-4 py-2 text-sm font-semibold text-brand-foreground shadow-brand hover:opacity-95"
        >
                  ...
                  <TableRow key={r.id}>
                    ...
                    <TableCell>
                      <div className="flex flex-wrap items-center gap-1.5">
                        <a
                          href="/templates/employment-services-agreement.pdf"
                          download={`Employment_Agreement_${(r.name || r.empCode || r.id).replace(/\s+/g, "_")}.pdf`}
                          className="inline-flex items-center gap-1 rounded-full border border-border bg-card px-2 py-1 text-[10px] font-semibold text-foreground hover:bg-muted"
                          title="Download Contract Template (PDF)"
                        >
                          <Download className="h-3 w-3" /> <span className="hidden xl:inline">PDF</span>
                        </a>
                        {!r.cancelled && (
                          <>
              ...
              <div key={r.id} className="rounded-2xl border border-border bg-card p-4 shadow-sm">
                ...
                <div className="mt-2 flex flex-wrap items-center gap-2">
                  <a
                    href="/templates/employment-services-agreement.pdf"
                    download={`Employment_Agreement_${(r.name || r.empCode || r.id).replace(/\s+/g, "_")}.pdf`}
                    className="inline-flex items-center gap-1 rounded-full border border-border bg-card px-2.5 py-1 text-[10px] font-semibold hover:bg-muted"
                    title="Download Contract Template (PDF)"
                  >
