...
function FinancePayroll() {
  ...
  const periodKey = ["finance", "payroll", year, month, dept] as const;
        ...
        <table className="w-full text-sm">
          <thead className="border-b border-border bg-muted/40">
            <tr className="text-[11px] uppercase tracking-wider text-muted-foreground">
              ...
              {isOn("basic_salary") && <SortTh sortKey="basic_salary" current={sortKey} dir={sortDir} onClick={toggleSort} align="end">{t("basicSalary") || "Basic Salary"}</SortTh>}
              {isOn("salary") && <SortTh sortKey="salary" current={sortKey} dir={sortDir} onClick={toggleSort} align="end">{t("grossSalary")}</SortTh>}
              {isOn("insurance_salary") && <SortTh sortKey="insurance_salary" current={sortKey} dir={sortDir} onClick={toggleSort} align="end">{t("insuranceSalary") || "Insurance Salary"}</SortTh>}
              {isOn("emergency_fund") && <SortTh sortKey="emergency_fund" current={sortKey} dir={sortDir} onClick={toggleSort} align="end">{t("emergencyFund") || "Emergency Fund"}</SortTh>}
              ...
              {isOn("advance_deduction") && <SortTh sortKey="advance_deduction" current={sortKey} dir={sortDir} onClick={toggleSort} align="end">Advances</SortTh>}
          ...
          <tbody>
            ...
            {paginated.map((r) => (
              <tr key={r.employee_id} className="border-b border-border last:border-b-0 hover:bg-muted/40">
                ...
                {isOn("salary") && <td className="px-4 py-3 text-end font-mono tabular-nums">{fmt(r.gross_salary ?? r.salary)}</td>}
                ...
                {isOn("advance_deduction") && <td className="px-4 py-3 text-end font-mono tabular-nums text-destructive">{r.advance_deduction ? `-${fmt(r.advance_deduction)}` : "—"}</td>}
          ...
          {filtered.length > 0 && (
            <tfoot className="border-t border-border bg-muted/40 font-semibold">
              <tr>
                ...
                {isOn("basic_salary") && <td className="px-4 py-3 text-end font-mono tabular-nums">{fmt(filtered.reduce((s, r) => s + (r.basic_salary ?? 0), 0))}</td>}
                {isOn("salary") && <td className="px-4 py-3 text-end font-mono tabular-nums">{fmt(filtered.reduce((s, r) => s + (r.gross_salary ?? r.salary), 0))}</td>}
                {isOn("insurance_salary") && <td className="px-4 py-3 text-end font-mono tabular-nums">{fmt(filtered.reduce((s, r) => s + (r.insurance_salary ?? 0), 0))}</td>}
                {isOn("emergency_fund") && <td className="px-4 py-3 text-end font-mono tabular-nums">{fmt(filtered.reduce((s, r) => s + (r.emergency_fund ?? 0), 0))}</td>}
