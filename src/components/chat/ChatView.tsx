...
export function ChatView({ mode = "admin" }: ChatViewProps) {
  const { t } = useI18n();
  ...
  const availableTabs: FilterTab[] = isManager
    ? ["all", "direct"]
    : ["all", "direct", "department", "broadcast"];

  return (
    <div className="flex h-[calc(100vh-5.5rem)] flex-col gap-4 overflow-hidden">
      {/* Top Header */}
      <div className="flex shrink-0 items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            {isManager
              ? "Direct chat with your team members."
          ...
          <div className="space-y-2 border-b border-border p-3">
            <div className="relative">
              ...
              <input
                ...
                placeholder={isManager ? "Search team chats…" : "Search conversations…"}
          ...
          <div className="flex-1 overflow-y-auto divide-y divide-border/40">
            ...
            {!channelsLoading && filteredChannels.length === 0 && (
              <div className="p-8 text-center text-xs text-muted-foreground">
                ...
                {isManager ? (
                  <>
                    No conversations found. Click <strong>New Message</strong> to start a chat with a team member.
                  </>
                ) : (
                  <>
          ...
          {activeChannel ? (
            ...
            <div className="my-auto flex flex-col items-center justify-center p-8 text-center">
              ...
              <p className="mt-1 max-w-sm text-xs text-muted-foreground">
                {isManager
                  ? "Choose a team member chat from the left, or click New Message to start a direct chat with your team."
                  : "Choose a direct chat or department group from the left, or click New Message to broadcast to a department."}
              </p>
            </div>
          )}
        </div>
  ...
  return (
    <div ref={containerRef} className="relative w-full">
      ...
      <button
        ...
        className="flex w-full items-center justify-between rounded-xl border border-border bg-background px-3 py-2.5 text-xs text-start transition-colors hover:border-brand focus:border-brand focus:outline-none"
      >
        <div className="flex min-w-0 flex-1 items-center gap-2">
          {Icon && (
            <div className="grid h-6 w-6 shrink-0 place-items-center rounded-lg bg-muted text-muted-foreground">
...
function NewMessageModal({
  ...
  const isManager = mode === "manager";
  const [targetType, setTargetType] = useState<"direct" | "department" | "broadcast">(
    isManager ? "direct" : "department"
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (targetType !== "broadcast" && !selectedTargetId) {
      return toast.error(
        isManager ? "Please select a team member" : `Please select a ${targetType === "department" ? "department" : "user"}`
      );
    }
      ...
      >
        <div className="flex items-center justify-between">
          <h3 className="font-display text-lg font-bold">
            {isManager ? "New Message to Team Member" : "New Message / Broadcast"}
          </h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {/* Target Type Selector - Only shown in admin mode */}
          {!isManager && (
            <div>
          ...
          {!isManager && targetType === "department" && (
            <div>
          ...
          {targetType === "direct" && (
            <div>
              ...
              <SearchableSelectPicker
                ...
                onChange={setSelectedTargetId}
                isLoading={isLoading}
                placeholder={isManager ? "— Choose a team member —" : "— Choose an employee —"}
                searchPlaceholder={
                  isManager
                    ? "Search team member by name or email…"
                    : "Search employee by name, email, or department…"
                }
                emptyText={isManager ? "No team members found" : "No active employees found"}
                icon={User}
          ...
          {/* Broadcast Note (admin only) */}
          {!isManager && targetType === "broadcast" && (
