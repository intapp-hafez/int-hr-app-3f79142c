  ...
  return (
    <div className="space-y-5">
      ...
      <section className="overflow-hidden rounded-2xl border border-border bg-card">
        ...
        <Link to="/employee/biometrics" className="block w-full text-start">
          <Row icon={Fingerprint} label="Face & fingerprint" />
        </Link>
      </section>
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/50 p-4" onClick={onClose}>
      ...
      >
        ...
        <PwdField label={t("currentPassword")} value={cur} onChange={setCur} />
        <PwdField label={t("newPassword")} value={next} onChange={setNext} />
        <PwdField label={t("confirmPassword")} value={conf} onChange={setConf} />
    ...
    <label className="block space-y-1.5">
      ...
      <div className="relative">
        <input
          ...
          className="w-full rounded-lg border border-input bg-background px-3 py-2 pr-10 text-sm outline-none focus:border-ring"
          autoComplete="off"
        />
        <button
          type="button"
