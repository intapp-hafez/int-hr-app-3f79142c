...
import { Plus, Search, Filter, X, ChevronRight, Upload, FileText, ArrowUp, ArrowDown, ArrowUpDown, Pencil, Trash2 } from "lucide-react";
...
function EmployeesPage() {
  ...
  const [sort, setSort] = useState<"full_name" | "email" | "created_at" | "status">("created_at");
                ...
                <Td>
                  <div className="flex items-center gap-2" onClick={(ev) => ev.stopPropagation()}>
                    <button
                      ...
                      className="inline-flex items-center gap-1 rounded-full border border-border bg-card px-2 py-1 text-xs font-semibold hover:bg-muted"
                    ...
                    <button
                      ...
                      title={isAdmin ? undefined : "Admin role required"}
                      className="inline-flex items-center gap-1 rounded-full border border-destructive/30 bg-destructive/10 px-2 py-1 text-xs font-semibold text-destructive hover:bg-destructive/20 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-destructive/10"
...
function SortableTh({ label, col, sort, dir, onSort }: {
  label: string; col: "full_name" | "email" | "created_at" | "status";
...
function AddEmployeeModal({ onClose }: { onClose: () => void }) {
  ...
  function submit(e: React.FormEvent) {
    ...
    if (!(VALID_DEPARTMENTS as readonly string[]).includes(form.dept)) return setErr("Invalid department");
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center overflow-y-auto bg-foreground/40 p-4 md:items-center" onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} className="my-auto w-full max-w-2xl rounded-3xl bg-background p-6 shadow-soft">
        ...
        <form onSubmit={submit} className="space-y-3 max-h-[65vh] overflow-y-auto pr-1">
          ...
          {tab === "personal" && (
            <div className="grid gap-3 md:grid-cols-2 border-t border-border pt-3 mt-2">
              ...
              <Field label={t("city")}><input value={form.city} onChange={(e) => upd("city", e.target.value)} className={inputCls} /></Field>
              <Field label={t("district")}><input value={form.district} onChange={(e) => upd("district", e.target.value)} className={inputCls} /></Field>
          ...
          {tab === "employment" && (
            <div className="grid gap-3 md:grid-cols-2">
              <Field label={t("department")}>
                <select value={form.dept} onChange={(e) => upd("dept", e.target.value)} className={inputCls}>
                  {["Engineering", "Sales", "HR", "Operations", "Design", "Finance", "Marketing"].map((d) => <option key={d}>{d}</option>)}
                </select>
              </Field>
              <Field label={t("position")}>
                <input value={form.position} onChange={(e) => upd("position", e.target.value)} maxLength={60} className={inputCls} />
              </Field>
...
type AddTab = "personal" | "employment" | "documents";
const ADD_TABS: { id: AddTab; labelKey: string }[] = [
  { id: "personal", labelKey: "tabPersonalAddress" },
  { id: "employment", labelKey: "tabEmploymentSalary" },
...
const VALID_DEPARTMENTS = ["Engineering", "Sales", "HR", "Operations", "Design", "Finance", "Marketing"] as const;
  ...
  async function handleFile(file?: File | null) {
    ...
    try {
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        const dept = String(r.dept ?? "Engineering");
        if (!(VALID_DEPARTMENTS as readonly string[]).includes(dept)) { invalid++; continue; }
