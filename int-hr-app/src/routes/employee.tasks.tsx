...
import { useStore, transitionTask, transitionTrip, selfAssignTask, getState, type TaskStatus, type TaskPriority } from "@/lib/store";
...
function EmployeeTasksPage() {
  ...
  const myTasks = tasks.filter((tk) => tk.assignees.includes(meId));
  const myTrips = trips.filter((tr) => tr.assignee === meId);
  ...
  const managerName = (id: string) => employees.find((e) => e.id === id)?.name ?? id;
              ...
              <li key={tk.id} className="rounded-2xl border border-border bg-card p-4 shadow-soft">
                ...
                <div className="mt-3 flex flex-wrap items-center gap-2">
                  ...
                  {tk.status !== "done" && tk.status !== "cancelled" && (
                    <button onClick={() => setPrompt({ kind: "task", id: tk.id, to: "done", label: t("taskCheckOut") })} className="inline-flex items-center gap-1 rounded-full bg-success/10 px-2.5 py-1 text-[11px] font-semibold text-success">
