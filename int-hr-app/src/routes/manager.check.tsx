...
function CheckPage() {
  ...
  // Manager-only: redirect any other role to their own panel.
...
function LeaveModal({ onClose, onSubmit }: { onClose: () => void; onSubmit: (d: { leave_type_id?: string | null; leave_type_name: string; start_date: string; end_date: string; days: number; paid: boolean; reason?: string; proof_url?: string | null; proof_mime?: string | null; proof_name?: string | null }) => Promise<void> }) {
  ...
  return (
      ...
      <div onClick={(e) => e.stopPropagation()} className="w-full max-w-md rounded-3xl bg-background p-5 shadow-soft">
        ...
        <form className="space-y-3" onSubmit={submit}>
          <select value={type} onChange={(e) => {
            ...
            const sel = (types as any[]).find((tp) => tp.name === e.target.value);
            if (sel) setPaid(!!sel.paid);
    ...
    {requiresProof && (
            <div>
              <p className="mb-1 text-xs font-medium text-muted-foreground">Doctor proof (PDF / PNG / JPEG, max 1.5 MB)</p>
