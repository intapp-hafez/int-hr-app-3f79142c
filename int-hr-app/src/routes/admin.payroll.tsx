...
import { Download, Wallet, TrendingUp, AlertTriangle, Search, Filter, Award, Lock, Unlock, FileText, Loader2, RefreshCw, ChevronLeft, ChevronRight, FileDown, ArrowUp, ArrowDown, ArrowUpDown } from "lucide-react";
...
function AdminPayroll() {
  ...
  type SortKey = "employee_name" | "department" | "salary" | "allowance" | "bonus" | "penalty" | "net_pay" | "kpi" | "daily_rate";
  const [sortKey, setSortKey] = useState<SortKey>("employee_name");
  const [sortDir, setSortDir] = useState<"asc" | "desc">("asc");
  function toggleSort(k: SortKey) {
    if (sortKey === k) setSortDir((d) => (d === "asc" ? "desc" : "asc"));
    else { setSortKey(k); setSortDir(k === "employee_name" || k === "department" ? "asc" : "desc"); }
  }
        ...
        <table className="w-full text-sm">
          <thead className="border-b border-border bg-muted/40">
            <tr className="text-[11px] uppercase tracking-wider text-muted-foreground">
              ...
              <SortTh sortKey="department" current={sortKey} dir={sortDir} onClick={toggleSort}>{t("department")}</SortTh>
              <SortTh sortKey="salary" current={sortKey} dir={sortDir} onClick={toggleSort} align="end">{t("salary")}</SortTh>
              <SortTh sortKey="daily_rate" current={sortKey} dir={sortDir} onClick={toggleSort} align="end">{t("dailyRate")}</SortTh>
              <Th align="center">P / L / A</Th>
              <SortTh sortKey="allowance" current={sortKey} dir={sortDir} onClick={toggleSort} align="end">{t("allowance")}</SortTh>
              <SortTh sortKey="bonus" current={sortKey} dir={sortDir} onClick={toggleSort} align="end">{t("bonus")}</SortTh>
              <SortTh sortKey="penalty" current={sortKey} dir={sortDir} onClick={toggleSort} align="end">{t("penalty")}</SortTh>
              <SortTh sortKey="net_pay" current={sortKey} dir={sortDir} onClick={toggleSort} align="end">{t("netPay")}</SortTh>
              <SortTh sortKey="kpi" current={sortKey} dir={sortDir} onClick={toggleSort} align="center">{t("kpi")}</SortTh>
              <Th align="center">Payslip</Th>
            </tr>
          </thead>
          <tbody>
            {isLoading && (
              <tr><td colSpan={11} className="p-8 text-center text-sm text-muted-foreground"><Loader2 className="mx-auto h-4 w-4 animate-spin" /></td></tr>
            )}
            {paginated.map((r) => (
              <tr key={r.employee_id} className="border-b border-border last:border-b-0 hover:bg-muted/40">
                <td className="px-4 py-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-muted-foreground">{r.emp_code ?? "—"}</p>
                    </div>
                  </div>
                </td>
                <td className="px-4 py-3">{r.department ?? "—"}</td>
                <td className="px-4 py-3 text-end font-mono tabular-nums">{fmt(r.salary)}</td>
                <td className="px-4 py-3 text-end font-mono tabular-nums text-muted-foreground">{fmt(r.daily_rate)}</td>
                ...
                <td className="px-4 py-3 text-end font-mono tabular-nums text-info">+{fmt(r.allowance)}</td>
                <td className="px-4 py-3 text-end font-mono tabular-nums text-success">+{fmt(r.bonus)}</td>
                <td className="px-4 py-3 text-end font-mono tabular-nums text-destructive">-{fmt(r.penalty)}</td>
                <td className="px-4 py-3 text-end font-mono tabular-nums font-semibold">{fmt(r.net_pay)}</td>
            ...
            {!isLoading && filtered.length === 0 && (
              <tr><td colSpan={11} className="p-8 text-center text-sm text-muted-foreground">No employees match the current filters for {periodLabel}.</td></tr>
            )}
          </tbody>
          {filtered.length > 0 && (
            <tfoot className="border-t border-border bg-muted/40 font-semibold">
              <tr>
                ...
                <td className="px-4 py-3 text-end font-mono tabular-nums">{fmt(totals.base)}</td>
                <td className="px-4 py-3" />
                <td className="px-4 py-3" />
                <td className="px-4 py-3 text-end font-mono tabular-nums text-info">+{fmt(totals.allowance)}</td>
                <td className="px-4 py-3 text-end font-mono tabular-nums text-success">+{fmt(totals.bonus)}</td>
                <td className="px-4 py-3 text-end font-mono tabular-nums text-destructive">-{fmt(totals.penalty)}</td>
                <td className="px-4 py-3 text-end font-mono tabular-nums">{fmt(totals.net)}</td>
                <td className="px-4 py-3 text-center">{avgKpi}%</td>
                <td className="px-4 py-3" />
              </tr>
            </tfoot>
          )}
