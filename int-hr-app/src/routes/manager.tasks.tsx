...
import { useStore, addTasksBatch, transitionTask, removeTask, getState, type TaskPriority, type TaskStatus, type ManagerTask } from "@/lib/store";
...
function ManagerTasksPage() {
  ...
  const team = useMemo(() => getTeam(employees, me), [employees, me]);
  ...
  return (
    <div className="space-y-4">
          ...
          {visible.map((tk) => (
            <li key={tk.id} className="rounded-2xl border border-border bg-card p-4 shadow-soft">
              ...
              <div className="mt-3 flex flex-wrap items-center gap-2">
                {tk.status === "pending" && (
                  <button onClick={() => me && transitionTask(tk.id, "in_progress", me.id)} className="inline-flex items-center gap-1 rounded-full border border-border bg-card px-2.5 py-1 text-[11px] font-semibold">
                    <Play className="h-3 w-3" /> {t("markInProgress")}
                  </button>
                )}
                {tk.status !== "done" && tk.status !== "cancelled" && (
                  <button onClick={() => me && transitionTask(tk.id, "done", me.id)} className="inline-flex items-center gap-1 rounded-full bg-success/10 px-2.5 py-1 text-[11px] font-semibold text-success">
                    <Check className="h-3 w-3" /> {t("markDone")}
                  </button>
                )}
                {tk.status !== "cancelled" && tk.status !== "done" && (
                  <button onClick={() => me && transitionTask(tk.id, "cancelled", me.id)} className="inline-flex items-center gap-1 rounded-full border border-border bg-card px-2.5 py-1 text-[11px] font-semibold text-muted-foreground">
                ...
                <button onClick={() => { removeTask(tk.id); toast.success("Removed"); }} className="ms-auto inline-flex items-center gap-1 rounded-full border border-border bg-card px-2.5 py-1 text-[11px] font-semibold text-danger">
      ...
      {open && me && <AddTaskModal me={me.id} team={team} onClose={() => setOpen(false)} />}
    </div>
  );
}
...
function AddTaskModal({ me, team, onClose }: { me: string; team: Array<{ id: string; name: string }>; onClose: () => void }) {
  const { t } = useI18n();
  ...
  const submit = () => {
    ...
    const created = addTasksBatch(cleaned.map((r) => ({
      title: r.title.trim(),
    ...
    toast.success(`${t("addTask")} (${created.length})`);
    onClose();
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center overflow-y-auto bg-foreground/40 p-4 md:items-center" onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} className="my-auto w-full max-w-2xl rounded-3xl bg-background p-5 shadow-soft">
